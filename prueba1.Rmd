...
df_tickets <- left_join(df_total_def,df_calles,by = "Factura_simpl")
df_tickets <- df_tickets %>% mutate(Parking_def = ifelse(is.na(Parking.x), Parking.y, Parking.x)) %>% select(!c(Parking.x, Parking.y)) %>% mutate(Parking=Parking_def) %>% select(!Parking_def)
...
# Hacemos intervalos
intervalos <- seq(from = as.POSIXct("09:00:00", format = "%H:%M:%S"), to = as.POSIXct("21:30:00", format = "%H:%M:%S"), by = "4 hours")
df_int <- as.data.frame(intervalos)
df_int2 <- separate(df_int, intervalos, into = c("Fecha", "Hora"), sep = " ")
df_int2<- select(df_int2, -Fecha)
#df_int2$Hora <- hms(df_int2$Hora)
...
# 4. ¿Existe diferencia en el tamaño del ticket de compra entre aquellos clientes que utilizan el servicio de parking del supermercado y aquellos que no lo utilizan?
