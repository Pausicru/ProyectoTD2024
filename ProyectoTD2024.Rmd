...
df_num_tickets_fruta <- unique(df_merge$Factura_simpl)

df_num_tickets_frutas <- data.frame(Numero_de_factura = numeros_de_factura_unicos)

# Contar el número de tickets que contienen frutas y verduras y los que no
tickets_con <- df_num_tickets_frutas %>%
  mutate(Frutas_Verduras = ifelse(Numero_de_factura %in% df_fruta_todos$Numero_de_factura, "Sin frutas/verduras", "Con frutas/verduras")) %>%
