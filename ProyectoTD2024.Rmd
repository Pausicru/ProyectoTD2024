...
df_calles <- separate(df_calles, Municipio, into = c("COD_Postal", "Municipio"), sep = " ")
```
...
df_tickets <- df_tickets %>% select(Total_euros) %>% mutate(across(where(is.numeric), metodos))

#Número de outliers detectados:
num_outliers <- df_tickets %>% select(contains("regla")) %>% summarise(across(everything(),~sum(.,na.rm = TRUE))) 


#Valores etiquetados como outliers:
sort(df_tickets$Total_euros[df_tickets$Total_euros_reglasigma])
sort(df_tickets$Total_euros[df_tickets$Total_euros_reglahampel])
sort(df_tickets$Total_euros[df_tickets$Total_euros_reglaboxplot])
sort(df_tickets$Total_euros[df_tickets$Total_euros_reglapercentil])

#Graficamos:
 df_tickets %>%
pivot_longer(cols = contains("regla"), names_to = "regla", values_to = "outliers")%>%
ggplot(aes(x = regla, y = Total_euros, color = outliers)) + geom_point(position = 'jitter') +
...
if (nrow(productos_mismo_nombre) > 0) {
  ...
  # Visualizar el nuevo dataframe con los precios comparados
  print(df_comparacion_precios)
} else {
  print("No se ha comprado el mismo producto en dos ubicaciones diferentes.")
}

# Caso productos por peso (frutas, verduras y pescados)
# Filtrar solo los productos de frutas y verduras
df_ubi_frutas_verduras <- df_fruta_pescados %>%
  select(Nombre, Eur_el_Kg)

# Filtramos los productos con el mismo nombre
peso_mismo_nombre <- df_ubi_frutas_verduras %>%
  group_by(Nombre) %>%
  filter(n() > 1)  # Filtrar solo aquellos productos con más de una ocurrencia
...
df_fruta_pescados %>% group_by(Nombre) %>% summarise(contador =  n())  %>% 
       ...
       subtitle = "Relación entre frutas y pescados, y su consumición",
       x = "Frutas y pescados",
       y = "Frecuencia",
       caption = "Los datos han sido extraídos de tickets de Mercadona")
...
df_productos_todos %>% group_by(Nombre) %>% summarise(contador =  n())  %>% 
       ...
       subtitle = "Relación entre productos y su consumición",
       x = "Productos",
       y = "Frecuencia",
       caption = "Los datos han sido extraídos de tickets de Mercadona")          
...
# 5. ¿Cuántos tickets están registrados en cada población?
 df_calles %>% group_by(COD_Postal)  %>% summarise(contador =  n(), avg = mean(Total_euros)) %>% ggplot(aes(x=COD_Postal, y= contador, fill = COD_Postal)) +  geom_bar(stat="identity") +
