...
ggplot(variabilidad_precios, aes(x = Nombre, y = desviacion_estandar, fill = Nombre)) +
  ...
  labs(title = "Productos con mayor variabilidad de precios",
       x = "Productos",
...
ggplot(df_resultado, aes(x = Total_euros, y = Porcentaje_frutas_verduras)) +
  ...
  labs(x = "Costo total del ticket", y = "% que supone el costo de frutas y verduras", title = "Regresión entre Ticket y Costo de Frutas y Verduras")
...
df_precios_ubi <- df_total_def %>%
  select(Nombre, Importe, Calle, Municipio)

# Filtramoslos productos con el mismo nombre
...
if (nrow(productos_mismo_nombre) > 0) {
  ...
  df_comparacion_precios <- productos_mismo_nombre %>%
    group_by(Nombre) %>%
    mutate(producto_1 = Importe[1], producto_2 = Importe[2]) %>%
...
##########
# Caso productos por peso (frutas, verduras y pesados)
