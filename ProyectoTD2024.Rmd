...
author: "Marta Molina, Iñaki Martín, Nerea Galera, Sergio Mut, Paula Sigüenza"
date: "`r Sys.Date()`"
...
## Análisis de valores atípicos en nuestro conjunto de interés:
  En este subapartado pasaremos a detectar datos o valores atípicos, ya sean NA's (valores perdidos) u outliers   (valores‘alejados’ del resto). 
  
  En primer lugar, respecto a los NA's, podemos ver que solamente existen en 4     columnas. Existen NA´s en la columna (vble.) PUnidad cuando se hace referencia a las       frutas y veduras y a pescados.
  Esto tiene sentido, ya que, ambos dos tienen un formato diferente al resto de productos, como ya hemos           comentado anteriormente. Además, existen NA´s en las columnas (vbles.) Kgs, Eur_el_kg y en Info cuando se hace   referencia al resto de productos que no sean ni frutas ni verduras ni pescados. Esto también tiene sentido, ya   que, complementa a lo anterior, es decir, el resto de productos no constan de ese tipo de formato.
  
  En segundo   lugar, para la detección de outliers sólo hace falta visualizar la gráfica (boxplot) dónde constan los 4        métodos vistos en la asignatura de Tratamiento de los Datos:
...
etiquetas_intervalos <- c("1", "2", "3")

intervalos_grafica <- c("9:00 a 12:59", "13:00 a 16:59", "17:00 a 21:30")
...
df_calles %>% group_by(intervalos) %>% summarise(contador =  n())  %>% ggplot(aes(x=intervalos, y= contador, fill = intervalos)) + geom_bar(stat = "identity") + labs(title = "Gráfica 1",
       ...
       scale_x_discrete(labels = intervalos_grafica)
...
# 4. ¿Existe diferencia en el tamaño del ticket de compra entre aquellos clientes
 ...
 df_calles  %>% group_by(Parking) %>% summarise(contador =  n())  %>% 
       ...
       subtitle = "Relación entre los clientes y el servicio Parking",
       x = "Parking",
       y = "Clientes",
       caption = "Los datos han sido extraídos de tickets de Mercadona")
...
FIGURA 8: ¿Existe diferencia en el tamaño del ticket de compra entre aquellos clientes que utilizan el servicio de parking del supermercado y aquellos que no lo utilizan?
  Como podemos observar, nos encontramos frente a otro gráfico de barras, representado con un bar_plot y          responde a nuestra pregunta con un rotundo sí. Esto se explica gracias a que el tamaño de la compra está     relacionado con si el cliente ha utilizado el servicio Parking o no, ya que se pueden dar factores como que haya aparcado su   vehículo relativamente cerca del supermercado, viva cerca del supermercado, etc.
