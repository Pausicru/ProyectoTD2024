...
df_tickets %>% select(Total_euros) %>% mutate(across(where(is.numeric), metodos))


#Número de outliers detectados:
df_tickets %>% select(contains("regla")) %>% summarise(across(everything(), ~sum(.,na.rm = T)))
...
ggplot(aes(x = regla, y = Total_euros, color = outliers)) + geom_point(position = 'jitter')#mejoramos la legibilidad del gráfico.
```
...
# 4. ¿Existe diferencia en el tamaño del ticket de compra entre aquellos clientes
 ...
 df_calles  %>% group_by(Parking) %>% summarise(contador =  n())  %>% 
       ...
       # ¿Existe diferencia entre el precio total de un ticket y el uso del servicio Perking?
