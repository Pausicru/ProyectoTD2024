...
ggplot(aes(x = regla, y = Total_euros, color = outliers)) + geom_point(position = 'jitter') +
  labs(title = "Número de outliers detectados por método",
       ...
       y = "Total_euros")
ggsave("./graficas/GraficaOutliers.pdf")
```
...
ggplot(variabilidad_precios, aes(x = Nombre, y = desviacion_estandar, fill = Nombre)) +
  ...
  labs(title = "Productos con mayor variabilidad de precios",
       ...
       caption = "Datos de precios extraídos de tickets de Mercadona")

# Los productos con mayor variabilidad de precios son:
cat(paste(variabilidad_precios$Nombre, ":", variabilidad_precios$desviacion_estandar, "\n"))
...
df_resultado$Porcentaje <- round((df_resultado$Costo_frutas_verduras / df_resultado$Total_euros) * 100, 2)
...
# 4. ¿Existe diferencia en el tamaño del ticket de compra entre aquellos clientes
 ...
 df_calles  %>% group_by(Parking) %>% summarise(contador =  n())  %>% 
       ...
       # ¿Existe diferencia entre el precio total de un ticket y el uso del servicio Parking?
        ...
        df_calles %>% group_by(Parking) %>% ggplot(aes(x=Parking, y= Total_euros, fill = Parking))  + geom_boxplot() + labs(title = "Gráfica 5.1 "   , subtitle = "Relación entre Total_euros y el servicio Parking",
       x = "Parking",
       y = "Total_euros",
       caption = "Los datos han sido extraídos de tickets de Mercadona")
